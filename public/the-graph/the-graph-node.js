(function (context) {
  ...
  TheGraph.Node = React.createFactory( React.createClass({
    ...
    componentDidMount: function () {
      ...
      domNode.addEventListener("dblclick", this.onDoubleClick);
    ...
    onDoubleClick: function(event){
      ...
      window.kathaa.latest_node_edit_focus = this.props.node;
    ...
    shouldComponentUpdate: function (nextProps, nextState) {
      ...
      //TO-DO: This is a rather hacky way to re render progress updates. Fix this
      try{
        if(this.props.node.metadata.refresh_progress==true){
          delete this.props.node.metadata.refresh_progress;
          return true;
        }
      }catch(err){
        console.log(err);
    ...
    render: function() {
      ...
      // var progress = parseInt(Math.random()*100);
      var progress;
      try{
        progress = this.props.node.metadata.progress == undefined ? 0 : parseInt(this.props.node.metadata.progress);
      }catch(err){
        console.log(err);
      }
      ...
      if(this.props.node.component == "core/sentence_input"){
        ...
        border.className = " node-border-rect-sentence-output";
      }
      ...
      if(this.props.node.component == "core/sentence_input"){
        ...
        innerRect.className += " node-bg-sentence-output";
      }


      var innerRectOptions = TheGraph.merge(innerRect, { width: this.props.width - 6, height: this.props.height - 6 });
      ...
      var progressbarLabelOptions = TheGraph.merge(TheGraph.config.node.progressLabelText, { x: this.props.width / 2, y: this.props.height - 90 , children: progress + "%" });
      var progressbarLabel = TheGraph.factories.node.createProgressLabelText.call(this, progressbarLabelOptions);


      var progressBarBackgroundOptions = TheGraph.merge(TheGraph.config.node.innerRect, { width: this.props.width - 6, height: 3, y: this.props.height - 80, fill:"red", stroke:"red" });
      progressBarBackgroundOptions.className = "drag";
      var progressBarBackground = TheGraph.factories.node.createProgressBarRect.call(this, progressBarBackgroundOptions);

      var progressBarForegroundOptions = TheGraph.merge(TheGraph.config.node.innerRect, { width: parseInt((progress/100)*(this.props.width - 6)), height: 3, y: this.props.height - 80 , fill:"#02e83c", stroke:"#02e83c"});
      ...
      if(window.kathaa.show_module_progress){
        ...
        nodeContents.push(progressBarBackground);
        nodeContents.push(progressBarForeground);
      }
